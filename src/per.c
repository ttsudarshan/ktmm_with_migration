/*
 * ktmm_vmscan.c (Kernel 6.1 with Bulk Migration Support)
 *
 * Page scanning and related functions.
 * NOW WITH BULK MIGRATION QUEUES (from kernel 5.14 implementation)
 */
...
#include <linux/migrate.h> // for migrate_pages()
...
/*****************************************************************************
 * Bulk Migration Configuration (Kernel 6.1 Adaptation)
 *****************************************************************************/
...
/*****************************************************************************
 * Bulk Migration Queue Structures (ADDED from kernel 5.14)
 *****************************************************************************/
...
static void page_stats_timer_callback(struct timer_list *t)
{
    ...
    /* Bulk migration stats */
    u64 promote_attempted = atomic64_read(&mig_attempted_promote);
    u64 promote_succeeded = atomic64_read(&mig_succeeded_promote);
    ...
    /* Calculate ratio */
    u64 ratio_num = 0, ratio_den = 1;
    if (demoted > 0) {
        ratio_num = promoted * 100;
        ratio_den = demoted;
    }

    printk(KERN_INFO "*** KTMM PAGE STATS: Total Promoted: %llu, Total Demoted: %llu (Ratio: %llu.%02llu:1) ***\n",
           promoted, demoted, ratio_num / ratio_den / 100, (ratio_num / ratio_den) % 100);
...
static int track_folio_access(struct folio *folio, struct pglist_data *pgdat, const char *location)
{
    ...
    if (was_accessed) {
        ...
        /* Immediately clear the bit after printing so we don't print it again */
...
/*****************************************************************************
 * File-backed Page Filter (ADDED from kernel 5.14)
 *****************************************************************************/
...
/*****************************************************************************
 * ALLOC & SWAP (ADAPTED for Kernel 6.1)
 *****************************************************************************/
...
/*****************************************************************************
 * Bulk Migration Worker Threads (ADDED from kernel 5.14, Kernel 6.1 compatible)
 *****************************************************************************/
 ...
 *
 * Based on worker_fn from kernel 5.14 implementation
 */
...
/*****************************************************************************
 * Queue Management Functions (ADDED from kernel 5.14)
 *****************************************************************************/

/**
 * queue_folio_for_promote - add a folio to the promotion queue
 * @folio: folio to queue for promotion (PMEM -> DRAM)
 *
 * Takes a reference on the folio and queues it for bulk migration
 * by the promote_worker thread.
 *
...
/**
 * scan_promote_list - scan promote lru folios for bulk migration
 *
                              ...
                              struct pglist_data *pgdat)
{
    ...
    /*
     * MIGRATION: Queue pages for bulk promotion (PMEM -> DRAM)
     * Instead of migrating one-by-one, we queue folios for the
     * promote_worker thread to handle in batches.
     */
    if (nr_taken > 0) {
        struct folio *folio, *next;
        ...
        list_for_each_entry_safe(folio, next, &l_hold, lru) {
            ...
            /* Remove from l_hold and queue for bulk migration */
            list_del_init(&folio->lru);
        ...
        if (nr_queued > 0) {
            printk(KERN_INFO "ktmm: pgdat %d QUEUED %lu folios for promotion (PMEM->DRAM)\n",
                   nid, nr_queued);
        }
    }
 ...
 * them up the to active list if they have been referenced.
 *
                      ...
                      struct mem_cgroup_reclaim_cookie *reclaim)
{
    ...
    do {
        ...
        for_each_evictable_lru(lru) {
            unsigned long nr_to_scan = 1024; /* Changed to 1024 for better page access detection */
...
static void tmemd_try_to_sleep(pg_data_t *pgdat, int nid)
{
    ...
    remaining = schedule_timeout(5 * HZ); /* 5 seconds for page access detection */
...
/**
 * tmemd_start_available - Start daemons and worker threads
 *
 * Daemons are only started on online/active nodes. They are
 * currently stored in a local array.
 *
 * ADDED: Bulk migration worker threads initialization
 */
int tmemd_start_available(void)
{
    ...
    /* Initialize bulk migration worker threads */
    promote_worker_task = kthread_run(promote_worker_fn, NULL, "ktmm_promote_worker");
    if (IS_ERR(promote_worker_task)) {
        printk(KERN_ERR "Failed to create promote worker thread\n");
        return PTR_ERR(promote_worker_task);
    }

    demote_worker_task = kthread_run(demote_worker_fn, NULL, "ktmm_demote_worker");
    if (IS_ERR(demote_worker_task)) {
        printk(KERN_ERR "Failed to create demote worker thread\n");
        kthread_stop(promote_worker_task);
        return PTR_ERR(demote_worker_task);
    }

    printk(KERN_INFO "KTMM bulk migration workers initialized\n");
...
/**
 * tmemd_stop_all - Stop all threads (daemons and workers)
 *
 * This stops all thread daemons for each node when exiting.
 * ADDED: Stop bulk migration worker threads
 */
void tmemd_stop_all(void)
{
    ...
    /* Print final bulk migration stats */
    printk(KERN_INFO "*** KTMM FINAL BULK MIGRATION STATS ***\n");
    ...
    /* Stop bulk migration worker threads */
    if (promote_worker_task) {
        kthread_stop(promote_worker_task);
        printk(KERN_INFO "Promote worker thread stopped\n");
    }

    if (demote_worker_task) {
        kthread_stop(demote_worker_task);
        printk(KERN_INFO "Demote worker thread stopped\n");
    }
