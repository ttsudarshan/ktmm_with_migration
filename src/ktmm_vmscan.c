/*
 * ktmm_vmscan.c
 * FIXED: Allocators now force specific NUMA nodes to prevent "migration to self" errors.
 */
 ...
 // Use GFP_NOIO to prevent filesystem deadlocks
 #define TMEMD_GFP_FLAGS GFP_NOIO
 
 // Global to store the PMEM node ID (usually 1)
 int pmem_node_id = 1; 
 int dram_node_id = 0;
 ...
 /*****************************************************************************
  * Statistics Counters (Defined here to avoid compile errors)
  *****************************************************************************/
 ...
 // Allocate specifically on PMEM node (Node 1)
 struct page* alloc_pmem_page(struct  page *page, unsigned long data)
 {
     // FIX: Using alloc_pages_node forces allocation on the specific node ID.
     // GFP_HIGHUSER_MOVABLE is standard for user pages.
   return alloc_pages_node(pmem_node_id, GFP_HIGHUSER_MOVABLE | GFP_NOIO, 0);
 }
 
 // Allocate specifically on DRAM node (Node 0)
 struct page* alloc_normal_page(struct page *page, unsigned long data)
 {
     return alloc_pages_node(dram_node_id, GFP_HIGHUSER_MOVABLE | GFP_NOIO, 0);
 }
 
 static struct page *ktmm_alloc_pages(gfp_t gfp_mask, unsigned int order, int preferred_nid,
           nodemask_t *nodemask)
 {
     // This is the hooked allocator, irrelevant for the migration worker
     // but kept for module completeness
 ...
 static int promote_worker_fn(void *arg)
 {
     ...
     current->flags |= PF_MEMALLOC; // Prevent recursion
   ...
   while (!kthread_should_stop()) {
     ...
     while (n < BATCH_N && !list_empty(&promote_queue)) {
       struct migrate_node *x = list_first_entry(&promote_queue, struct migrate_node, link);
     ...
     for (i = 0; i < n; i++) {
       ...
       list_add_tail(&x->folio->lru, &migrate_list);
       kfree(x);
     }
 
     // Promote: Migrate to DRAM
     ret = migrate_pages(&migrate_list, alloc_normal_page, NULL,
             0, MIGRATE_SYNC, MR_NUMA_MISPLACED,
             &nr_succeeded);
 
         if (ret != 0 && nr_succeeded == 0) {
             // Print error once to debug
             static int err_print_count = 0;
             if (err_print_count++ < 5) 
                 pr_err("KTMM: Promote failed ret=%d\n", ret);
         }
 ...
 static int demote_worker_fn(void *arg)
 {
   ...
   while (!kthread_should_stop()) {
     ...
     while (n < BATCH_N && !list_empty(&demote_queue)) {
       struct migrate_node *x = list_first_entry(&demote_queue, struct migrate_node, link);
       list_del_init(&x->link);
     ...
     ret = migrate_pages(&migrate_list, alloc_pmem_page, NULL,
         ...
         if (ret != 0 && nr_succeeded == 0) {
              static int err_print_count = 0;
              if (err_print_count++ < 5) 
                  pr_err("KTMM: Demote failed ret=%d\n", ret);
         }
           ...
           struct pglist_data *pgdat)
 {
   ...
   if (pgdat->pm_node == 0 && pmem_node_id != -1) {
     ...
     list_for_each_entry_safe(folio, next, &folio_list, lru) {
             ...
             // Allow all pages (anonymous and file) to migrate for testing
       list_del_init(&folio->lru);
 ...
 int tmemd_start_available(void) 
 {
   ...
   for_each_online_node(nid)
   {
     ...
     if (nid == 1) {
       ...
       set_pmem_node(nid);
             pmem_node_id = nid; // Ensure global var matches
     }
