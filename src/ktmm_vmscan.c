 ...
 * ========================= */
 ...
 static bool ktmm_zone_watermark_ok_safe(struct zone *z, unsigned int order,
         ...
         struct pglist_data *pgdat)
 {
   ...
   nr_taken = ktmm_isolate_lru_folios(nr_to_scan, lruvec, &l_hold, &nr_scanned, sc, lru);
         ...
         struct pglist_data *pgdat)
 {
   ...
   while (!list_empty(&l_hold)) {
     ...
     if (unlikely(buffer_heads_over_limit)) {
       if (ktmm_folio_needs_release(folio) && folio_trylock(folio)) {
     ...
     struct mem_cgroup_reclaim_cookie *reclaim)
 {
   ...
   u64 scan_start_time, total_scan_time_us;
   ...
   total_scan_time_us = (ktime_get_ns() - scan_start_time) / 1000;
   
   printk(KERN_INFO "*** SCAN_STATS (Node %d): Scanned: %lu, Time: %llu us, Promoted: %lld, Demoted: %lld ***\n", 
 ...
 static int tmemd(void *p) 
 {
   ...
   struct scan_control sc = {
     ...
     .reclaim_idx = MAX_NR_ZONES - 1, .only_promote = 1,
   };
 
   if(!cpumask_empty(cpumask)) set_cpus_allowed_ptr(task, cpumask);
